...
public class FinalStatsWindow extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabelSecondsText10.setText("(confidence level of 95%)");
    ...
    public void setFinalStats(int repetitions, int totalSimConnections ,double execModAvgTotal, double transacModAvgTotal, double querycModAvgTotal, double processModAvgTotal, double connectionAvgLifeTime, double[] connectionAvgLifeTimeVector,double avgRejectedConnections, double ddlAvgTimeExec, double ddlAvgTimeTrans, double ddlAvgTimeQuery, double ddlAvgTimeProcess, double updateAvgTimeExec, double updateAvgTimeTrans, double updateAvgTimeQuery, double updateAvgTimeProcess, double joinAvgTimeExec, double joinAvgTimeTrans, double joinAvgTimeQuery, double joinAvgTimeProcess, double selectAvgTimeExec, double selectAvgTimeTrans, double selectAvgTimeQuery, double selectAvgTimeProcess) {
        ...
        if (repetitions > 1) {
            double XValue = connectionAvgLifeTime;
            double S2Value;
            double sum = 0;
            for (int iterator = 0; iterator < repetitions; iterator = iterator + 1) {
                sum += Math.pow((connectionAvgLifeTimeVector[iterator] - XValue), 2);
            }
            S2Value = sum / (repetitions - 1);
            TDistribution tDistrib = new TDistribution(repetitions - 1);
            // Using a 0.95 confidence level
            double tValue = tDistrib.inverseCumulativeProbability(1 - ((1 - 0.95) / 2));
            double result = tValue * Math.sqrt(S2Value / repetitions);
            jLabelConfidenceIntervalValue.setText("[" + decimalFormat.format(XValue - result) + " , " + decimalFormat.format(XValue + result) + "]");
        }
        else {
            jLabelConfidenceIntervalValue.setText("Only one value");
            jLabelSecondsText10.setVisible(false);
        }
